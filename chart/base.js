...
var Chart = Backbone.Model.extend({
    ...
    */
    read: function (opts, cb) {
        ...
        me.zip.file(opts.file).async("text").then(function (content) {
            var parser = new xml2js.Parser({explicitArray: false});
            parser.parseString(content, function (err, o) {
                if (err) {
    ...
    */
    write: function (opts) {
        ...
        me.zip.file(opts.file, new Buffer(xml)).async("base64").then(function (content) {
        });
    },
    /*
        Get column name
    ...
    */
    removeUnusedCharts: function (o) {
        ...
        } else if (me.charts ["column"] && !me.charts ["bar"]) {
        ...
        async.series([
            function (cb) {
                ...
                fs.readFile(__dirname + "/../template/" + me.tplName + ".xlsx", function (err, data) {
                    ...
                    ;
                    // me.zip.load (data);
                    me.zip.loadAsync(data).then(function (zip) {
                        cb();
                    });
                    // cb ();
                });
            ...
            me.zip.generateAsync({type: me.type}).then(function (blob) {
                cb(null, blob);
            });
        });
    }
